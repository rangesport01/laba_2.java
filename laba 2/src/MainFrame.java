...
public class MainFrame extends JFrame {
    ...
    public Double calculate1(Double x, Double y, Double z) {
        return (Math.pow(Math.log(1+x)*Math.log(1+x) + Math.cos(Math.PI*Math.pow(z,3)),Math.sin(y)) +Math.pow(Math.pow(Math.E,x*x) + Math.cos(Math.pow(Math.E,z)) + Math.sqrt(1./y),1./x));
    }

    public Double calculate2(Double x, Double y, Double z) {
        return (Math.pow(Math.cos(Math.pow(y,2)),3) / Math.pow(z,1/x))*x;
    }
